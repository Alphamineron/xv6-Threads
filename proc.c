...
growproc(int n)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if (p != curproc && p->pgdir == curproc->pgdir)     // Grow Thread Memory Too
        p->sz = sz;
  }
...
// ========================================================
  ...
  *(sp) = 0xffffffff;
  // ========================================================
  // ABOVE CODE BASICALLY SETS UP THE SAME STACK STRUCTURE IN MEMORY BUT
  // IT DOESN'T INTERFERE WITH THE PARENT PGDIR DIRECTLY, USES THE STACK PTR
  // GENERATED BY thread_create, 
  // FREED BY thread_join,
  // STORED IN struct proc.
  // ========================================================
  ...
  // ustack[2] = (uint)arg2;
  // stack -= 12;
  // if(copyout(np->pgdir, (uint)stack, ustack, 12) < 0)
    // return -1;
// ========================================================
// ========================================================
// ========================================================
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
      ...
      if(p->state == ZOMBIE){
        ...
        *stack = p->ustack;
        ...
        kfree(p->kstack);
        p->kstack = 0;
        // freevm(p->pgdir); Don't Free The Shared pgdir
        p->pid = 0;
        p->parent = 0;
        p->name[0] = 0;
        p->killed = 0;
